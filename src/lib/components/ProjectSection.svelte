...
<section class="py-16 md:py-24 lg:py-32 px-6 md:px-12 lg:px-24 max-w-7xl mx-auto">
		...
		<div class="grid grid-cols-1 lg:grid-cols-5 gap-10 lg:gap-16 items-center">
			...
			<div class="{reversed ? 'lg:order-1' : 'lg:order-2'} lg:col-span-2">
				<ScrollReveal delay={150}>
					...
					<h2 class="font-display text-3xl md:text-4xl lg:text-5xl tracking-tight">
						{project.name}
					</h2>
					{#if project.quote}
					...
					{#if project.description}
						<p class="mt-4 text-cream/70 leading-relaxed">
							{project.description}
						</p>
					{/if}
					{#if project.members}
						<ul class="mt-6 space-y-1">
							{#each project.members as member}
								<li class="text-sm">
									<span class="text-cream/90">{member.name}</span>
						...
						<div class="mt-6">
							...
							<ul class="space-y-1">
								{#each project.albums as album}
									<li class="text-sm">
										<span class="text-cream/70 italic">{album.title}</span>
										<span class="text-muted/60 text-xs ml-1">({album.year})</span>
									</li>
								{/each}
							</ul>
						</div>
	...
	{:else}
		...
		<ScrollReveal>
			<div class="text-center max-w-xl mx-auto">
				...
				<h2 class="font-display text-2xl md:text-3xl lg:text-4xl tracking-tight">
					{project.name}
				</h2>
				{#if project.links.length > 0}
